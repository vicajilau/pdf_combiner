...
static void pdf_combiner_plugin_handle_method_call(PdfCombinerPlugin *self,
  ...
  if (strcmp(method, "mergeMultiplePDF") == 0) {
    ...
    response =
        create_pdf_from_multiple_images(fl_method_call_get_args(method_call));
...
FlMethodResponse *merge_multiple_pdfs(FlValue *args) {
  if (fl_value_get_type(args) != FL_VALUE_TYPE_MAP) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "invalid_arguments", "Expected a map with inputPaths and outputPath",
        nullptr));
  }

  // Get inputPaths (List<String>)
  FlValue *input_paths_value = fl_value_lookup_string(args, "paths");
  if (!input_paths_value ||
      fl_value_get_type(input_paths_value) != FL_VALUE_TYPE_LIST) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
  ...
  for (int i = 0; i < num_pdfs; i++) {
    ...
    if (!path_value || fl_value_get_type(path_value) != FL_VALUE_TYPE_STRING) {
      return FL_METHOD_RESPONSE(fl_method_error_response_new(
          "invalid_arguments", "Each item in inputPaths must be a string",
          nullptr));
    }
  ...
  if (!new_doc) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "document_creation_failed", "Failed to create new PDF document",
        nullptr));
  }
...
FlMethodResponse* create_image_from_pdf(FlValue* args) {
    ...
    if (!doc) {
      ...
      return FL_METHOD_RESPONSE(fl_method_error_response_new(
          ...
          ("Failed to load document: " + input_path).c_str(), nullptr));
    }
    ...
    if (!FPDF_ImportPages(new_doc, doc, nullptr, total_pages)) {
      ...
      FPDF_CloseDocument(new_doc);
      return FL_METHOD_RESPONSE(fl_method_error_response_new(
          "page_import_failed", "Failed to import page into new document",
          nullptr));
    }
    total_pages += page_count;
  ...
  if (!FPDF_SaveAsCopy(new_doc, (FPDF_FILEWRITE *)&file_write,
    ...
    FPDF_CloseDocument(new_doc);
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "document_save_failed", "Failed to save the new PDF document",
        nullptr));
  }
...
FlMethodResponse *create_pdf_from_multiple_images(FlValue *args) {
  ...
  if (!max_width_value ||
    ...
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "invalid_arguments", "width must be an int", nullptr));
  }
  ...
  if (!max_height_value ||
    ...
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "invalid_arguments", "maxHeight must be an int", nullptr));
  }
  ...
  // Get keepAspectRatio (Bool)
  FlValue *keep_aspect_ratio_value =
      fl_value_lookup_string(args, "keepAspectRatio");
  if (!keep_aspect_ratio_value ||
      fl_value_get_type(keep_aspect_ratio_value) != FL_VALUE_TYPE_BOOL) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "invalid_arguments", "keepAspectRatio must be a boolean", nullptr));
  }
  ...
  for (const auto &input_path : input_paths) {
    ...
    if (!image_data) {
      ...
      return FL_METHOD_RESPONSE(fl_method_error_response_new(
          ...
          ("Failed to load image: " + input_path).c_str(), nullptr));
    }
    ...
    if (max_width != 0 || max_height != 0) {
      ...
      if (!stbir_resize_uint8_linear(image_data, width, height, 0,
        ...
        FPDF_CloseDocument(new_doc);
        return FL_METHOD_RESPONSE(fl_method_error_response_new(
            "image_resize_failed", "Failed to resize image", nullptr));
      }
    ...
    if (!new_page) {
      ...
      return FL_METHOD_RESPONSE(fl_method_error_response_new(
          ...
          ("Failed to create page for image: " + input_path).c_str(), nullptr));
    }
...
FlMethodResponse *create_image_from_pdf(FlValue *args) {
  if (fl_value_get_type(args) != FL_VALUE_TYPE_MAP) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "invalid_arguments",
        "Expected a map with inputPath, outputDirPath, width, height, "
        "compression and createOneImage keys",
        nullptr));
  }

  // Get params from the map
  const char *input_path =
      fl_value_get_string(fl_value_lookup_string(args, "path"));
  ...
  if (!max_height_value ||
      fl_value_get_type(max_height_value) != FL_VALUE_TYPE_INT) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "invalid_arguments", "height must be an int", nullptr));
  }
  ...
  if (!compression_value ||
      fl_value_get_type(compression_value) != FL_VALUE_TYPE_INT) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "invalid_arguments", "compression must be an int", nullptr));
  }
  ...
  const char *output_path =
      fl_value_get_string(fl_value_lookup_string(args, "outputDirPath"));
  if (!input_path || !output_path) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "invalid_arguments", "Missing path or outputDirPath", nullptr));
  }
  ...
  FlValue *create_one_image_value =
      fl_value_lookup_string(args, "createOneImage");
  if (!create_one_image_value ||
      fl_value_get_type(create_one_image_value) != FL_VALUE_TYPE_BOOL) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "invalid_arguments", "createOneImage must be a boolean", nullptr));
  }
  ...
  if (create_one_image) {
    ...
    if (!combined_bitmap) {
      FPDF_CloseDocument(doc);
      return FL_METHOD_RESPONSE(fl_method_error_response_new(
          "bitmap_creation_failed", "Failed to create combined bitmap",
          nullptr));
    }
    ...
    for (int i = 0; i < page_count; ++i) {
      ...
      FlValue *image_path_value =
          fl_value_new_string(output_image_path.c_str());
      fl_value_append(result, image_path_value);
...
void pdf_combiner_plugin_register_with_registrar(FlPluginRegistrar *registrar) {
  ...
  g_autoptr(FlMethodChannel) channel =
      fl_method_channel_new(fl_plugin_registrar_get_messenger(registrar),
                            "pdf_combiner", FL_METHOD_CODEC(codec));
